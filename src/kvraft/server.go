...
func (kv *KVServer) Get(args *GetArgs, reply *GetReply) {
	...
	//	reply.Value = kv.stateMachine.GetL(args.Key)
	//	if reply.Value == nil {
	//		reply.Err = ErrNoKey
	//	}
	//	return
	//}
	tid := fmt.Sprintf("%d_Get_%d", args.CID, args.TaskId)
	op := Op{
		TID:    tid,
		Method: "Get",
		Key:    args.Key,
	...
	if _, _, isleader := kv.rf.Start(op); !isleader {
		...
		case <-wait:
			kv.mu.Lock()
			reply.Value = kv.stateMachine.GetL(op.Key)
			if reply.Value == nil {
				reply.Err = ErrNoKey
			}
...
func (kv *KVServer) PutAppend(args *PutAppendArgs, reply *PutAppendReply) {
	// Your code here.
	//DPrintf("server[%d] get %s request", kv.me, args.Op)
	kv.mu.Lock()
	defer kv.mu.Unlock()

	reply.Err = OK
	opname := fmt.Sprintf("%d_%s", args.CID, args.Op)
	tid := fmt.Sprintf("%d_%s_%d", args.CID, args.Op, args.TaskId)
